	...
	SubShader
	{
		...
		/*	#pragma multi_compile_fwdbase_fullshadows
			...
			v2f vert (appdata v)
			{
				...
				float idx = _IndexBuffer[v.vertexId];// +v.instanceId * _NumVertexOfPerTorus;
				...
				float indexOfSegment = idx / 10;

				float tt = fmod(_Time.y * 0.5 + v.instanceId * 0.1 + (indexOfSegment / (_MaxSegment + 1)) * 0.11, 1);// (sin(_Time.y * 0.5 + v.instanceId * 0.001 + (indexOfSegment / (_MaxSegment + 1)) * 0.001) + 1) * 0.5;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				/// Final Color:
				float3 finalColor = 1 * lerp(i.color * 1.3, i.color * 0.1, i.uv.x);// (i.color * i.uv.x);
